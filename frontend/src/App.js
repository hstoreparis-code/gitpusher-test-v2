...
import ForNocodeBuildersPage from "./pages/for-nocode-builders";
import ForNocodeBuildersFRPage from "./pages/for-nocode-builders-fr";
import ForNocodeBuildersESPage from "./pages/for-nocode-builders-es";
import ForNocodeBuildersPTPage from "./pages/for-nocode-builders-pt";
...
function Landing({ t, lang, setLang, dark, setDark, currentLang, languages, isLoadingLang, forceSignupMode = false, onOpenDemoModal }) {
  const { token } = useAuth();
...
function AuthCard({ t, onSuccess, onClose, initialTab = "login" }) {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const detail = err?.response?.data?.detail;
      const msg = typeof detail === "string" ? detail : detail?.msg || "Signup failed";
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <Card className="bg-slate-900/70 border-white/10 shadow-2xl shadow-cyan-500/20 text-slate-50 relative">
      ...
      <CardHeader>
        <CardTitle className="text-base font-semibold text-center">
          ...
          <span className="bg-gradient-to-r from-cyan-400 to-cyan-600 bg-clip-text text-transparent">Pusher.AI</span>
        </CardTitle>
      ...
      <CardContent>
        ...
        {/* Bouton Accès Admin sous les providers */}
        <div className="mt-5 flex flex-col gap-3 items-stretch">
          <Button
            className="w-full justify-center rounded-full bg-gradient-to-r from-violet-500 to-purple-500 hover:from-violet-400 hover:to-purple-400 text-white text-xs sm:text-sm font-semibold px-3 py-2 flex items-center gap-2 shadow-[0_0_18px_rgba(139,92,246,0.85)]"
            onClick={() => navigate("/admin-login")}
            data-testid="auth-admin-dashboard-button"
          >
            ⚙️ Accès Admin
          </Button>
        </div>
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 w-full max-w-full sm:max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-8 space-y-3 sm:space-y-8 overflow-x-hidden">
        {/* Dashboard Title + bannière pub compacte (desktop) + Toggle Free/Premium (mobile) */}
        <div className="flex flex-col gap-2 mb-2 sm:mb-3">
          <div className="flex items-center justify-between">
            ...
            {(currentPlan === "free" || currentPlan === "freemium" || currentPlan === "demo") && (
              <div className="hidden lg:flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-slate-900/80 border border-cyan-500/30 shadow-[0_0_18px_rgba(34,211,238,0.5)] text-[11px] lg:w-[470px] justify-end">
                ...
                <Button
                  size="sm"
                  className="h-7 px-3 rounded-full bg-gradient-to-r from-cyan-500 via-sky-500 to-cyan-400 text-slate-950 text-[11px] font-semibold shadow-[0_0_18px_rgba(34,211,238,0.8)] hover:shadow-[0_0_24px_rgba(34,211,238,1)]"
          ...
          {/* Toggle Free / Premium (mobile, sous le titre) - Visible uniquement pour utilisateurs Free/Freemium */}
          ...
          <Card className="bg-slate-900/80 border-2 border-dashed border-violet-500/60 shadow-[0_0_24px_rgba(139,92,246,0.4)] rounded-xl overflow-hidden relative">
            ...
            {user?.plan && (
              <div className="absolute top-2 sm:top-3 right-4 z-20">
                <span className={`px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full text-[10px] sm:text-[11px] font-bold uppercase tracking-wide sm:tracking-wider shadow-lg ${
                  (user.plan || '').toLowerCase() === 'free' || (user.plan || '').toLowerCase() === 'freemium' || (user.plan || '').toLowerCase() === 'demo'
            ...
            <CardContent className="p-5 sm:p-6 relative z-10">
              <div className="flex items-start sm:items-center gap-4 sm:gap-6">
                ...
                <div className="relative flex-shrink-0 mt-4 sm:mt-0">
                  ...
                  <div className={`relative w-28 h-28 sm:w-28 sm:h-28 rounded-full ${
                    (user?.plan || '').toLowerCase() === 'premium' || (user?.plan || '').toLowerCase() === 'business'
                ...
                <div className="flex flex-col gap-2 flex-1 min-w-0 pt-2 sm:pt-0">
                  ...
                  <div className="flex items-center gap-2">
                    {((user?.plan || '').toLowerCase() === 'premium' || (user?.plan || '').toLowerCase() === 'business') ? (
                      <div className="flex items-center gap-1.5 px-2 py-0.5 sm:px-5 sm:py-3 rounded-full bg-gradient-to-r from-violet-500/20 to-purple-500/20 border border-violet-400/40">
                        <span className="text-sm sm:text-2xl">✨</span>
                        <span className="text-[10px] sm:text-base font-semibold text-violet-300">Accès illimité</span>
                      </div>
                    ) : credits != null && credits <= 2 ? (
                ...
                {selected && (
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      size="xs"
                      className="h-8 px-3 rounded-full border-violet-400/60 bg-violet-500/10 text-violet-300 hover:bg-violet-500/20 hover:text-violet-200 hover:border-violet-400 shadow-[0_0_12px_rgba(139,92,246,0.3)] hover:shadow-[0_0_18px_rgba(139,92,246,0.6)] transition-all text-[10px]"
...
function TermsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-50 flex flex-col">
      ...
      <main className="flex-1">
        <div className="max-w-4xl mx-auto px-4 py-8 sm:py-10 space-y-8 text-sm sm:text-base">
          ...
          {/* Bloc 1 : Mentions légales / Informations société (France) */}
          <section className="space-y-2">
            <h2 className="text-base sm:text-lg font-semibold text-slate-100">1. Informations légales</h2>
            <p className="text-slate-300">
              Conformément à la législation française, les informations suivantes sont mises à disposition des
              utilisateurs du service <strong>GitPusher™.AI</strong>.
            </p>
          ...
          <section className="space-y-2">
            ...
            <p className="text-slate-300">
              Les présentes conditions générales d'utilisation encadrent l'accès et l'utilisation du service
              <strong> GitPusher™.AI</strong>.
            </p>

            <div className="space-y-2">
              <h3 className="text-sm sm:text-base font-semibold text-slate-100">2.1 Objet du service</h3>
              <p className="text-slate-300">
            ...
            <div className="space-y-2">
              <h3 className="text-sm sm:text-base font-semibold text-slate-100">2.2 Création de compte et connexions OAuth</h3>
              <p className="text-slate-300">
            ...
            <div className="space-y-2">
              <h3 className="text-sm sm:text-base font-semibold text-slate-100">2.3 Données traitées</h3>
              <p className="text-slate-300">
            ...
            <div className="space-y-2">
              <h3 className="text-sm sm:text-base font-semibold text-slate-100">2.4 Accès à vos dépôts Git</h3>
              <p className="text-slate-300">
              ...
              <ul className="list-disc list-inside text-slate-300 space-y-1">
                <li>
                  Créer de nouveaux dépôts sous votre compte ou votre organisation, selon les permissions accordées.
                </li>
                <li>
            ...
            <div className="space-y-2">
              <h3 className="text-sm sm:text-base font-semibold text-slate-100">2.5 Responsabilités</h3>
              <p className="text-slate-300">
            ...
            <div className="space-y-2">
              <h3 className="text-sm sm:text-base font-semibold text-slate-100">2.6 Sécurité</h3>
              <p className="text-slate-300">
            ...
            <div className="space-y-2">
              <h3 className="text-sm sm:text-base font-semibold text-slate-100">2.7 Conservation et suppression</h3>
              <p className="text-slate-300">
            ...
            <div className="space-y-2">
              <h3 className="text-sm sm:text-base font-semibold text-slate-100">2.8 Modifications des conditions</h3>
              <p className="text-slate-300">
          ...
          {/* Bloc 3 : Politique de contact / réclamation */}
          <section className="space-y-2">
            <h2 className="text-base sm:text-lg font-semibold text-slate-100">3. Contact &amp; réclamations</h2>
            <p className="text-slate-300">
              Pour toute question concernant ces mentions légales, les conditions générales d'utilisation ou le
              traitement de vos données, vous pouvez contacter l'administrateur de GitPusher™.AI via le chat de support
              intégré ou par le canal habituel mis à votre disposition.
            </p>
          </section>
        </div>
      </main>
...
// Duplicate AccountPage function removed - active implementation below
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <Card className="bg-slate-900/70 border-cyan-500/30">
          <CardHeader>
            <CardTitle className="text-sm sm:text-base">🔑 Tokens Git Providers</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-xs text-slate-400">
              Connectez vos providers Git via Personal Access Token pour plus de contrôle
            </p>
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className="h-8 text-xs bg-slate-950/60 border-slate-700 font-mono"
                />
              </div>

              <div className="space-y-1">
                <Label className="text-xs flex items-center gap-2">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.546 10.93L13.067.452c-.604-.603-1.582-.603-2.188 0L8.708 2.627l2.76 2.76c.645-.215 1.379-.07 1.889.441.516.515.658 1.258.438 1.9l2.658 2.66c.645-.223 1.387-.078 1.9.435.721.72.721 1.884 0 2.604-.719.719-1.881.719-2.6 0-.539-.541-.674-1.337-.404-1.996L12.86 8.955v6.525c.176.086.342.203.488.348.713.721.713 1.883 0 2.6-.719.721-1.889.721-2.609 0-.719-.719-.719-1.879 0-2.598.182-.18.387-.316.605-.406V8.835c-.217-.091-.424-.222-.6-.401-.545-.545-.676-1.342-.396-2.009L7.636 3.7.45 10.881c-.6.605-.6 1.584 0 2.189l10.48 10.477c.604.604 1.582.604 2.186 0l10.43-10.43c.605-.603.605-1.582 0-2.187"/></svg>
              ...
              <div className="space-y-1">
                <Label className="text-xs">Bitbucket App Password</Label>
                <Input
                  type="password"
                  placeholder="ATBBxxxxxxxxxxxx"
              ...
              <Button
                size="sm"
                className="w-full rounded-full bg-gradient-to-r from-cyan-500 to-violet-500 hover:from-cyan-400 hover:to-violet-400 text-slate-950 text-xs font-semibold"
                onClick={async () => {
                  setSavingTokens(true);
                  setError("");
                  setSuccess("");
                  try {
                    await axios.post(`${API}/v1/auth/github`, 
                      { githubToken },
                      { headers: { Authorization: `Bearer ${token}` } }
                    );
                    setSuccess("✅ Tokens enregistrés avec succès");
                    setTimeout(() => setSuccess(""), 3000);
                  } catch (err) {
                    setError(err?.response?.data?.detail || "Erreur enregistrement tokens");
                    setTimeout(() => setError(""), 3000);
                  } finally {
                    setSavingTokens(false);
                  }
                }}
                disabled={savingTokens || (!githubToken && !gitlabToken && !bitbucketToken)}
              >
                {savingTokens ? "Enregistrement..." : "💾 Enregistrer les Tokens"}
              </Button>
          ...
          <section className="grid gap-6 md:grid-cols-3">
            ...
            <Card className="bg-gradient-to-br from-violet-500/10 via-slate-900/70 to-slate-900/70 border-violet-500/30 hover:border-violet-500/50 transition-all duration-300 relative shadow-[0_0_40px_rgba(139,92,246,0.35)]">
              ...
              <CardHeader>
                ...
                <p className="text-2xl font-semibold text-slate-300 mt-2">29,99€</p>
                <p className="text-xs text-emerald-400">≈ 0,60€ par crédit • 25 uploads</p>
              </CardHeader>
              <CardContent className="space-y-4">
            ...
            <Card className="bg-slate-900/70 border-amber-500/30 hover:border-amber-500/50 transition-all duration-300 shadow-[0_0_40px_rgba(251,191,36,0.35)]">
              <CardHeader>
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-bold text-amber-300">200</span>
                  <span className="text-slate-400 text-sm">crédits</span>
                </div>
                <p className="text-2xl font-semibold text-slate-300 mt-2">49,99€</p>
                <p className="text-xs text-emerald-400">≈ 0,25€ par crédit • 100 uploads</p>
              </CardHeader>
              <CardContent className="space-y-4">
                <ul className="space-y-2 text-sm text-slate-400">
                  <li className="flex items-start gap-2">
          ...
          <section className="space-y-6">
            ...
            >
              <CardHeader>
                ...
                <div className="flex items-baseline gap-2 mt-3">
                  <span className="text-2xl font-semibold text-amber-300">À partir de 99€</span>
